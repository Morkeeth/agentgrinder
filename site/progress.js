...
window.GrinderProgress = function ({client: db, me, app, frame, status, signIn}) {
  ...
  async function historyView() {
    ...
    try {
      ...
      $('progress-body').innerHTML=`${pending.length||notes.length?`<div class="return-actions"><section class="panel reply-form"><h2>I tried this. Show me what changed.</h2>${pending.length?pending.map(a=>`<p><a href="/?practice=${a.practice_id}#attempt-${a.id}">${esc(a.practice?.title || 'Review your practice')}</a><br><small>${esc(a.practice?.instruction || 'Your baseline is saved. Record the next session, then review what happened.')}</small></p>`).join(''):'<p>Choose a run and try one change in the next session.</p><a class="act" href="/?onboard">Record another run</a>'}</section><section class="panel reply-form"><h2>Responses to your work</h2>${notes.length?notes.map(n=>`<p><a href="${n.run_id?'/?run='+n.run_id:'/?inbox'}">${esc(n.kind==='ack'?'An ACK on your run':n.kind==='reply'?'A reply on your run':'A new follower')}</a><br><small>${esc(date(n.created_at))}</small></p>`).join(''):'<p>No unread responses. Share a run when you want someone else to see it.</p>'}<a href="/?inbox">Open Inbox</a></section></div>`:""}<div class="cta"><a class="act blue" href="/?onboard">Record a run</a></div><form id="history-filter" class="history-filters"><label>Find a run<input name="query" type="search" placeholder="Title or project you shared"></label><label>Harness<select name="harness" aria-label="Harness"><option value="">All harnesses</option></select></label><label>Audience<select name="audience" aria-label="Audience"><option value="">All audiences</option><option value="private">Only me</option><option value="link">Link only</option><option value="public">Public</option><option value="crew">Crew members</option></select></label><button type="submit">Find runs</button></form><p id="history-count" class="meta"></p><div id="history-list"></div><button id="history-more" class="act" ${more?'':'hidden'}>Load older runs</button><p><a href="/?onboard">Record another run</a></p>`;
      function render() {
