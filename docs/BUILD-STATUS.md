...
## Continued forum lifecycle and public link previews

Built titled questions tied to a run, answers, in-app subscriptions, recent-reply indicators, author-chosen answers and answer-to-practice links. Questions inherit the run audience. Editing a chosen answer clears the selection. Mark-read uses the last displayed reply rather than the wall clock, so a reply arriving after the screen loaded is not silently marked read. Server tests cover ownership, changed retries, cross-run answers, direct mutation denial, private-audience transitions, blocked authors, source deletion and anonymous subscription denial. Browser fixtures drive chosen-answer, follow, mark-read and practice-link controls; phone and desktop screens were inspected.

**Not deployed:** the new forum lifecycle requires `2026-09-05-forum.sql`. The browser connection repeatedly lost its tab or timed out, and no local authenticated Supabase/psql CLI is installed. Oscar was asked to reconnect Brave and open the project SQL editor. The existing live Forum remains at its previous version until the migration is applied. The new frontend is retained in Git, not included in this production stage.

**Deployed:** public share links now use `/r/RUN_ID`, with per-run Open Graph metadata and a generated 1200×630 PNG at `/api/run`. The server uses the existing public key, requests only public records and excludes notes/project names. HTML and image responses use no-store; this does not retract copies already cached by third-party social platforms. A real public run returned HTML 200 and image/png 200, the downloaded PNG was inspected, and an invalid ID returned 404. Public-filter/missing-record/title-escaping checks and actual PNG rendering passed locally. The PNG renderer is pinned to tested @vercel/og 0.8.5; 1.0.2 failed its local Node import.

Production deployment: `dpl_ABAkhqBVUuQJCjCeh6G6wBM6xT8K`. It contains the prior live site plus updated sharing links and the new server endpoints, package files and rewrites. It intentionally excludes the pending forum frontend and schema. Verification checks the staged bytes rather than claiming the entire working tree is deployed.
